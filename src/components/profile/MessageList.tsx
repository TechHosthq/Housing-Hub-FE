...
export default function MessageList({ viewMode, selectedId, onThreadSelect }: MessageListProps) {
    ...
    const ThreadList = () => (
        <div className={`bg-white dark:bg-gray-900 rounded-[22px] border border-[#F2F2F2] dark:border-gray-800 p-6 shadow-sm h-fit min-h-[500px] ${viewMode === 'chat' ? 'w-[340px] hidden md:block' : 'flex-1'}`}>
            <h2 className="text-[18px] font-black text-[#1A1A1A] dark:text-gray-100 font-montserrat mb-6 ml-2">
                Messages
            </h2>
            ...
            <div className="relative mb-6">
                ...
                <input
                    ...
                    className="w-full pl-11 pr-5 py-3 rounded-full border border-[#F2F2F2] dark:border-gray-800 bg-white dark:bg-gray-900 text-[12px] font-bold text-[#1A1A1A] dark:text-gray-100 placeholder:text-gray-300 dark:placeholder:text-gray-600 focus:outline-none focus:border-primary-dark transition-all"
                />
            </div>
            ...
            {isLoadingConvs ? (
                ...
                <div className="flex flex-col gap-1">
                    {filteredConversations.length === 0 ? (
                        <p className="text-center text-gray-400 dark:text-gray-500 text-xs py-10">No conversations found</p>
                    ) : (
                        filteredConversations.map((conv) => (
                            <div
                                ...
                                className={`p-4 rounded-2xl flex items-start gap-3 cursor-pointer transition-all group relative border-l-4 ${selectedId === conv.id
                                    ? "bg-white dark:bg-gray-900 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border-primary-dark"
                                    : "border-transparent hover:bg-gray-50 dark:hover:bg-gray-800/50"
                                    }`}
                            >
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500`}>
                                    <User size={20} />
                                </div>

                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start mb-0.5">
                                        <h3 className="text-[13px] font-black text-[#1A1A1A] dark:text-gray-100 truncate pr-2">
                                    ...
                                    <p className="text-[11px] font-bold text-[#666666] dark:text-gray-400 truncate">
                                        {conv.lastMessage}
                                    </p>
                                </div>
            ...
            <div className="flex-1 bg-white dark:bg-gray-900 rounded-[22px] border border-[#F2F2F2] dark:border-gray-800 shadow-sm flex flex-col h-full overflow-hidden">
                {activeChat ? (
                    <>
                                            ...
                                            >
                                                <div className={`max-w-[70%] space-y-1`}>
                                                    <div className={`px-5 py-4 rounded-[12px] text-[13px] leading-relaxed shadow-sm ${isOutgoing
                                                        ...
                                                        : "bg-white dark:bg-gray-900 border border-[#F2F2F2] dark:border-gray-800 text-[#1A1A1A] dark:text-gray-100"
                                                        }`}>
                                                        {msg.content}
                                                    </div>
                                                    <div className={`flex items-center gap-1 text-[10px] font-bold text-gray-400 dark:text-gray-500 ${isOutgoing ? "justify-end" : "justify-start"}`}>
                        ...
                        <div className="p-8 bg-white dark:bg-gray-900 border-t border-[#F2F2F2] dark:border-gray-800 shrink-0">
                            <form onSubmit={handleSendMessage} className="relative">
                                <input
                                    ...
                                    className="w-full pl-6 pr-16 py-4 rounded-xl border border-[#F2F2F2] dark:border-gray-800 bg-white dark:bg-gray-900 text-[13px] font-bold text-[#1A1A1A] dark:text-gray-100 placeholder:text-gray-300 dark:placeholder:text-gray-600 focus:outline-none focus:border-primary-dark transition-all"
                                />
                                <button
                                    type="submit"
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-white dark:bg-gray-900">
                        ...
                        <h3 className="text-[18px] font-black text-[#1A1A1A] dark:text-gray-100 font-montserrat mb-2">
                            Select a conversation
                        </h3>
                        <p className="text-[12px] text-[#666666] dark:text-gray-400 font-medium max-w-[240px]">
