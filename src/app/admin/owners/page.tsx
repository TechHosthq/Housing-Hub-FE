...
export default function AdminOwnersPage() {
    ...
    return (
        <div className="flex flex-col gap-8" onClick={() => setOpenDropdownId(null)}>
            ...
            />
            <h1 className="text-[28px] font-bold text-[#1A1A1A] dark:text-gray-100 font-montserrat" onClick={(e) => e.stopPropagation()}>Manage Owners</h1>
            ...
            <div className="flex gap-3 w-full" onClick={(e) => e.stopPropagation()}>
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full pl-12 pr-4 py-3.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-full text-[14px] focus:ring-2 focus:ring-[#0095FF]/10 outline-none transition-all"
                        value={searchQuery}
            ...
            <div className="flex flex-col gap-4 w-full">
                {paginatedOwners.map((owner) => (
                    <div key={owner.id} className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-[20px] p-6 shadow-sm hover:shadow-md transition-all group relative">
                        <div className="flex items-center gap-5">
                            ...
                            <div className="flex-1 flex flex-col gap-1.5">
                                <div className="flex items-center gap-3">
                                    <h3 className="text-[18px] font-bold text-[#1A1A1A] dark:text-gray-100 group-hover:text-[#0095FF] transition-colors">
                                        {owner.name}
                                    </h3>
                                ...
                                <div className="flex items-center gap-6 text-[14px] font-medium text-gray-500 dark:text-gray-500">
                            ...
                            <div className="relative" onClick={(e) => e.stopPropagation()}>
                                <button
                                    ...
                                    className={`p-2 rounded-full transition-colors ${openDropdownId === owner.id ? "bg-gray-100 dark:bg-gray-800 text-[#1A1A1A] dark:text-gray-100" : "text-gray-400 dark:text-gray-500 hover:text-[#1A1A1A] hover:bg-gray-50 dark:hover:bg-gray-800/50"}`}
                                >
                                    <MoreVertical size={20} />
                                </button>

                                {openDropdownId === owner.id && (
                                    <div className="absolute top-full right-0 mt-2 w-[220px] bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-[16px] shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                                        <button className="w-full text-left px-6 py-3 text-[15px] font-medium text-[#1A1A1A] dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                            View Details
                                        </button>
                                        <Link
                                            href={`/admin/kyc-review/${owner.id}`}
                                            className="w-full text-left block px-6 py-3 text-[15px] font-medium text-[#1A1A1A] dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"
                                        >
                                            Review KYC
                                        </Link>
                                        <div className="h-[1px] bg-gray-50 dark:bg-gray-800/50 my-1 mx-2" />
                                        <button
